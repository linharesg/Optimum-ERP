jQuery(function() {
    ...
    const $initialSuppliers = $("#supplierproduct_set-INITIAL_FORMS");
    let $supplierRow = $(".supplierRow")
    const $originalSupplier = $suppliersContainer.children(".row:first").clone(true);

    const updateFormIndex = function(){
        $(".supplierRow").each(function(index){
    ...
    $suppliersContainer.on("click", ".remove-btn", function(){
        ...
        if (parseInt($initialSuppliers.val()) > 0 && !($button.attr("data-url").includes("/0/"))) {
